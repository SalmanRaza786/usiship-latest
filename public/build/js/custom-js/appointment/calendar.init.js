...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var addNewOrderModal = new bootstrap.Modal(document.getElementById('addOrderModal'), {
        keyboard: false
    });
    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function (info) {
            ...
            fnGetOrderDetail(info.event.id);


           // document.getElementById("edit-event-btn").removeAttribute("hidden");
            document.getElementById('btn-save-event').setAttribute("hidden", true);
            //document.getElementById("edit-event-btn").setAttribute("data-id", "edit-event");
            //document.getElementById("edit-event-btn").innerHTML = "Edit Buttn";
            eventClicked();
            flatPickrInit();
        ...
        u_event = "<div class='card mb-3'>\
                        <div class='card-body'>\
                            <div class='d-flex mb-3'>\
                                ...
                                <div class='flex-shrink-0'><small class='badge badge-soft-primary ms-auto'>" + e_time_s  + " to "+e_time_e+"</small></div>\
                            </div>\
...
function fnGetOrderDetail(orderId){
    ...
    $.ajax({
        url: route('admin.orders.info',{id:orderId}),
        type: 'GET',
        async: false,
        dataType: 'json',
        dta:{orderId:orderId},
        success: function(response) {
            ...
            $('#whDock').text(response.data.dock);
            $('#orderDate').text(response.data.order_date);
            $('#slotFrom').text(response.data.slot_from);
            $('#slotTo').text(response.data.slot_to);
            $('.wh-name').text(response.data.ware_house);
            $('#currentStatus').text(response.data.status);
            $('#currentStatus').addClass(response.data.text_class);
...
$('#btn-detail').click(function() {
    var orderId = $('input[name=hidden_order_id]').val();
    window.location.href = route('admin.orders.detail',{id:orderId});
});
